...
async function showMainMenu(chatId) {
  ...
  const kb = { inline_keyboard: [
    ...
    [{ text: "🎞 Текст → фото → видео", callback_data: "story2video" }],
...
function showPromptGenMenu(chatId, msgId = null) {
  ...
  const kb = { inline_keyboard: [
    ...
    [{ text: "🎞 Файл → фото → видео", callback_data: "story2video" }],
    [{ text: "◀️ Назад", callback_data: "back_menu" }],
  ]};
  if (msgId) bot.editMessageText(text, { chat_id: chatId, message_id: msgId, parse_mode: "Markdown", reply_markup: kb }).catch(() => {});
  else bot.sendMessage(chatId, text, { parse_mode: "Markdown", reply_markup: kb });
...
async function imageResultItemToInputRef(item) {
  ...
  throw new Error("В результате изображения нет data/download_path");
}

async function sendStoryPipelineImage(chatId, imageResult, sceneNo, total, promptPair, model) {
  const caption = `🖼 *Story image ${sceneNo}/${total}*\n${md(model.label)}\n📝 _${md(cut(promptPair.image_prompt, 120))}_`;
  for (const item of imageResult.results || []) {
    let media = null;
    if (item.data) media = { type: "data_uri", value: item.data, mediaType: item.type || "image" };
    else if (item.download_path) {
      const url = `${STORAGE_URL}${item.download_path.startsWith("/") ? "" : "/"}${item.download_path}`;
      media = { type: "url", value: url, mediaType: item.type || "image" };
    }
    if (media) await sendV5Media(chatId, media, withSeedCaption(caption, item, imageResult.pollResult, imageResult.requestSeed));
  }
}
...
async function runStoryToVideoPipeline(chatId, storyText, filename = "story.txt") {
  ...
  for (let i = 0; i < imageSlots.length; i++) {
    try {
      ...
      const firstImage = (imageResult.results || []).find(item => item.data || item.download_path);
...
function showStoryPipelineMenu(chatId, msgId = null) {
  ...
  const kb = { inline_keyboard: [
    ...
    [{ text: "🔢 Своё число сцен", callback_data: "story_count_custom" }],
    [{ text: "📄 Загрузить файл и запустить", callback_data: "story_upload_file" }],
    [{ text: "🎨 Модель фото", callback_data: "open_imgmodel" }, { text: "🎥 Модель видео", callback_data: "open_vidmodel" }],
    [{ text: "📐 Соотношение", callback_data: "open_ratio" }],
    [{ text: "◀️ Назад", callback_data: "back_menu" }],
  ]};
  if (msgId) return bot.editMessageText(text, { chat_id: chatId, message_id: msgId, parse_mode: "Markdown", reply_markup: kb }).catch(() => {});
  return bot.sendMessage(chatId, text, { parse_mode: "Markdown", reply_markup: kb });
...
bot.on("callback_query", async (query) => {
  ...
  if (data.startsWith("story_count_")) {
    ...
    if (value === "custom") {
      ...
      return edit("🔢 Введи количество сцен от 1 до 200:", { inline_keyboard: [[{ text: "❌ Отмена", callback_data: "story2video" }]] });
    }
  ...
  if (data === "story_upload_file") {
    ...
    return edit("📄 Отправь .txt или .docx файл. После загрузки бот сразу запустит: текст → промпты → изображения → видео.", { inline_keyboard: [[{ text: "❌ Отмена", callback_data: "story2video" }]] });
  }
